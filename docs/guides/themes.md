...
languages; a `.sublime-syntax` in `~/.config/ycode/syntaxes/` adds one, and
a few extensions without a grammar of their own — `ts`, `toml`, `kt`,
`swift`, `zig` — borrow the nearest.
